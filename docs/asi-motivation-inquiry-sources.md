...
- 原対話00の「ASIにとって資源獲得は何のためか」「少ない変更で多くを知る」という問いを中心にした。目的の固定や善性への必然的収束を前提にしない。
- 原レポート01〜04にある「唯一」「必然」「論理的に不可能」といった保証は採用せず、条件付きの設計候補として検討した。M/Uを観測権限M・執行権限Uとする01独自の再定義は採用していない。
